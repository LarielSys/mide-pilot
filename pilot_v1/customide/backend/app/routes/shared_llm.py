...
def _resolve_generate_url() -> tuple[str | None, str, str | None]:
    ...
    if env_generate_url:
        return env_generate_url, "env.CUSTOMIDE_OLLAMA_GENERATE_URL", None

    env_legacy_url = _normalize_generate_url(os.environ.get("CUSTOMIDE_OLLAMA_URL", ""))
    if env_legacy_url:
        return env_legacy_url, "env.CUSTOMIDE_OLLAMA_URL", None
    ...
    for bucket_name, bucket in (("services", raw), ("services.services", services_obj), ("services.services.ollama", ollama)):
        for key in ("ollama_generate_url", "ollama_generate", "ollama_proxy_generate", "ollama_url", "ollama_base_url", "proxy_endpoint"):
            ...
            if isinstance(value, str) and value.strip():
                url = _normalize_generate_url(value)
                return url, f"{bucket_name}.{key}", None

    # Last fallback keeps local development functional when no env/config is present.
    local_fallback = _normalize_generate_url(settings.ollama_base_url)
    if local_fallback:
        return local_fallback, "settings.ollama_base_url", None
...
def llm_chat(payload: ChatRequest) -> dict[str, Any]:
    target, source_key, reason = _resolve_generate_url()
    model = payload.model or "qwen2.5-coder:7b"
