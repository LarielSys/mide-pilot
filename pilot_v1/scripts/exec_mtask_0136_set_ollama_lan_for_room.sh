...
ASK=$(curl -sf -X POST http://localhost:7070/rooms/OPS-CENTRAL/ask-ai \
  ...
  -d '{"text":"MTASK-0136 validation ping","caller":"worker"}' 2>&1 || echo "ASK_FAIL")
...
cat > "$RESULT_FILE" <<EOF
{
  ...
  "summary": "Set mide-chat Ollama base URL to Ubuntu LAN endpoint and validate AI room replies.",
...
echo "[${TASK_ID}] done: ROOM_OLLAMA_LAN_${STATUS^^}"
