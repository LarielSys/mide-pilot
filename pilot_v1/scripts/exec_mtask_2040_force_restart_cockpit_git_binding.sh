...
log "Patched backend files"

# Ensure old processes are removed so restarted services run patched code.
pkill -f "uvicorn app.main:app" || true
pkill -f "http.server 5570" || true
sleep 1

if command -v lsof >/dev/null 2>&1; then
  lsof -ti tcp:5555 | xargs -r kill -9 || true
  lsof -ti tcp:5570 | xargs -r kill -9 || true
fi

if [[ -f "$STACK_SCRIPT" ]]; then
  log "Starting CustomIDE stack"
  bash "$STACK_SCRIPT" >/tmp/mtask2040-start-local-stack.log 2>&1 || true
fi

log "Waiting for backend health"
for _ in $(seq 1 25); do
  if curl -fsS "$BACKEND_URL/health" >/tmp/mtask2040-health.json 2>/dev/null; then
    break
  fi
  sleep 2
done

curl -fsS "$BACKEND_URL/health" >/tmp/mtask2040-health.json

http_code=$(curl -s -o /tmp/mtask2040-git-status.json -w "%{http_code}" "$BACKEND_URL/api/git/status" || true)
if [[ "$http_code" != "200" ]]; then
  echo "api_git_status_http=$http_code"
  echo "--- backend log tail ---"
  tail -n 120 /tmp/customide-backend.log || true
  exit 1
fi

curl -fsS "$BACKEND_URL/api/status/sync-health" >/tmp/mtask2040-sync-health.json

python3 - <<'PY'
import json
from pathlib import Path

git_status = json.loads(Path('/tmp/mtask2040-git-status.json').read_text(encoding='utf-8'))
sync = json.loads(Path('/tmp/mtask2040-sync-health.json').read_text(encoding='utf-8'))
