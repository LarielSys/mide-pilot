...
def _resolve_generate_url() -> str:
    ...
    # Direct generate URLs if present in normalized or nested config.
...
uvicorn app.main:app --host 127.0.0.1 --port 5555 >/tmp/mtask0046r1-backend.log 2>&1 &
BACK_PID=$!
popd >/dev/null

pushd "${FRONTEND_ROOT}" >/dev/null
python3 -m http.server 5570 >/tmp/mtask0046r1-frontend.log 2>&1 &
FRONT_PID=$!
popd >/dev/null
...
git commit -m "customide: fix shared llm config resolution (MTASK-0046-RETRY1)" || true
