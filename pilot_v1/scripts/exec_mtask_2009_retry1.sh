#!/usr/bin/env bash
# MTASK-2009-RETRY1 — Deploy fun-copy cockpit (funhost) on port 8095
# Fixed: use git fetch + reset --hard instead of git pull to avoid local changes conflict
set -uo pipefail

TASK_ID="MTASK-2009-RETRY1"
...
echo "[${TASK_ID}] Starting fun-host deploy (retry1)..."
echo "[${TASK_ID}] serve_dir=${SERVE_DIR}"
echo "[${TASK_ID}] port=${PORT}"

# Sync to absolute latest — avoids local changes conflict
cd "${REPO_ROOT}"
...
if [[ ! -d "${SERVE_DIR}" ]]; then
  ...
  echo "[${TASK_ID}] ls_pilot=$(ls ${REPO_ROOT}/pilot_v1/ 2>/dev/null | tr '\n' ' ')"
