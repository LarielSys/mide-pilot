...
# Tight reins preflight: fail fast on dirty tree before runtime checks.
if [[ -n "$(git status --porcelain)" ]]; then
  echo "error=dirty_tree_before_validation"
  exit 1
fi
...
echo "reins_preflight_mode_check=info"
...
uvicorn app.main:app --host 127.0.0.1 --port 5555 >/tmp/mtask0045-backend.log 2>&1 &
BACK_PID=$!
popd >/dev/null

pushd "${FRONTEND_ROOT}" >/dev/null
python3 -m http.server 5570 >/tmp/mtask0045-frontend.log 2>&1 &
FRONT_PID=$!
popd >/dev/null
...
# Validate remote endpoint with worker config resolution.
...
# Validate structured failure mode when user disables config and omits host/user.
...
# Accept either success or a structured SSH failure for config-based remote check,
# as long as target resolution occurred and endpoint responded.
...
git commit -m "customide: validate remote target resolution and tighten reins (MTASK-0045)" || true
